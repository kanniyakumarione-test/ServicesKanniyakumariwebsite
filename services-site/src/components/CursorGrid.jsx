...
const CursorGrid = ({
  cellSize = 70,
  color = '#d97706', // Adjusted to amber-600 by default to match the brand
  radius = 140,
  falloff = 'smooth',
  holdTime = 400,
  fadeDuration = 800,
  ...
  useEffect(() => {
    // If it's a mobile device, bypass the animation loop entirely to save CPU/GPU.
    ...
    // Hardcap DPR to 1 to ensure butter smooth scrolling and zero lag on 4K/Retina displays
    const dpr = 1;
    ...
    const draw = now => {
      const p = propsRef.current;
      // Cap dt to 16ms max to prevent death spiral on heavy lag frames
      const dt = Math.min(now - lastFrame, 16);
      lastFrame = now;
  ...
  return (
    <div ref={containerRef} className={`relative h-full w-full overflow-hidden${className ? ` ${className}` : ''}`}>
      {!isMobile && <canvas ref={canvasRef} className="block h-full w-full pointer-events-none" />}
    </div>
  );
};
