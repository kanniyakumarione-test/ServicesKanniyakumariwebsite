...
const OptionWheel = ({
  ...
  useEffect(() => {
    ...
    const onWheel = e => {
      ...
      // Dampen the scroll step and invert the direction to match physical wheel dragging
      const step = Math.max(-0.5, Math.min(0.5, delta * -0.008));
  ...
  return (
    <div
      ...
      className={`relative h-full w-full select-none overflow-hidden outline-none font-display [touch-action:none] ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}${className ? ` ${className}` : ''}`}
      style={{
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          } ${selectedIndex === index ? 'font-bold uppercase tracking-widest' : 'font-extralight'}`}
